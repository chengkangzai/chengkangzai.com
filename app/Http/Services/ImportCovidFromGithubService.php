        ...
        return collect(explode(PHP_EOL, Http::get(self::url['HOSPITAL'])))
            ...
            ->map(function ($record) {
                ...
                return [
                    ...
                    'beds_covid' => (!isset($dailyCase[3]) || $dailyCase[3] == '') ? 0 : $dailyCase[3],
                    'beds_noncrit' => (!isset($dailyCase[4]) || $dailyCase[4] == '') ? 0 : $dailyCase[4],
                    'admitted_pui' => (!isset($dailyCase[5]) || $dailyCase[5] == '') ? 0 : $dailyCase[5],
                    'admitted_covid' => (!isset($dailyCase[6]) || $dailyCase[6] == '') ? 0 : $dailyCase[6],
                    'admitted_total' => (!isset($dailyCase[7]) || $dailyCase[7] == '') ? 0 : $dailyCase[7],
                    'discharged_pui' => (!isset($dailyCase[8]) || $dailyCase[8] == '') ? 0 : $dailyCase[8],
                    'discharged_covid' => (!isset($dailyCase[9]) || $dailyCase[9] == '') ? 0 : $dailyCase[9],
                    'discharged_total' => (!isset($dailyCase[10]) || $dailyCase[10] == '') ? 0 : $dailyCase[10],
                    'hosp_covid' => (!isset($dailyCase[11]) || $dailyCase[11] == '') ? 0 : $dailyCase[11],
                    'hosp_pui' => (!isset($dailyCase[12]) || $dailyCase[12] == '') ? 0 : $dailyCase[12],
                    'hosp_noncovid' =>(!isset($dailyCase[13]) || $dailyCase[13] == '') ? 0 : $dailyCase[13] ,
                    'created_at' => now(),
                    'updated_at' => now(),
                ];
            });
