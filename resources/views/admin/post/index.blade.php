...
@section('content')
    ...
    <div class="flex w-full flex-row-reverse overflow-hidden">
        <x-button class="my-1 text-base" :href="route('admin.posts.create')">
            {{ __('Create Post') }}
        </x-button>
    </div>
                            ...
                            <tr>
                                <td class="whitespace-no-wrap border-b border-gray-200 px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="text-sm font-medium leading-5 text-gray-900">{{ $post->title }}
                                            </div>
                                            <div class="text-sm leading-5 text-gray-500">
                                                <span class="inline-block">{{ $post->tags_count }}
                                                    {{ __('Tags') }}</span>
                                                <span class="inline-block">{{ $post->comments_count }}
                                                    {{ __('Comment') }}</span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <td
                                    ...
                                    <x-button :href="route('admin.posts.show', $post)" bg="bg-green-600" hoverBg="hover:bg-green-500">
                                        {{ __('View') }}
                                    </x-button>
                                    <x-button :href="route('admin.posts.edit', $post)">
                                        {{ __('Edit') }}
                                    </x-button>
                                    <form action="{{ route('admin.posts.destroy', $post) }}" class="m-0 inline-block p-0"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <x-input type="submit"
