...
@section('content')
    ...
    <div class="flex w-full flex-row-reverse overflow-hidden">
        <x-button class="my-1 text-base" :href="route('admin.roles.create')">
            {{ __('Create Role') }}
        </x-button>
    </div>
                        ...
                        @foreach ($roles as $role)
                            <tr>
                                <td class="whitespace-no-wrap border-b border-gray-200 px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 flex-shrink-0">
                                            <img src="https://ui-avatars.com/api/?name={{ $role->name }}&background=random&size=128"
                                                alt="avatar" class="h-10 w-10 rounded-full object-cover">
                                        </div>

                                        <div class="ml-4">
                                            <div class="text-sm font-medium leading-5 text-gray-900">{{ $role->name }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td
                                    ...
                                    <x-button :href="route('admin.roles.show', $role)" bg="bg-green-600" hoverBg="hover:bg-green-500">
                                        {{ __('View') }}
                                    </x-button>
                                    <x-button :href="route('admin.roles.edit', $role)">
                                        {{ __('Edit') }}
                                    </x-button>
                                    <form action="{{ route('admin.roles.destroy', $role) }}" class="m-0 inline-block p-0"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <x-input type="submit"
