                    ...
                    @foreach($permissions as $permission)
                        <tr>
                            <td class="px-6 py-2 whitespace-no-wrap border-b border-gray-200">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10">
                                        <img
                                            src="https://ui-avatars.com/api/?name={{ $permission->name }}&background=random&size=128"
                                            alt="avatar" class="h-10 w-10 rounded-full object-cover">
                                    </div>

                                    <div class="ml-4">
                                        <div
                                            class="text-sm leading-5 font-medium text-gray-900">{{ $permission->name }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                <button
                                    wire:click="$emit('openModal', 'admin.permission.show-modal', {{ json_encode(["permission" => $permission->id]) }})"
